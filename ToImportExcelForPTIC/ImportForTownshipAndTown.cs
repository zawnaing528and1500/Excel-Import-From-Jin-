...
namespace ToImportExcelForPTIC
{
    public partial class ImportForTownshipAndTown : Form
    {
        ...
        private void readDataFormExcel()
        {
            ...
            Excel.Range range, rangeEdu, rangeWHis, rangeParent, rangeSpouse, rangeTown, rangeTownship;
            ...
            for (rCnt = 1; rCnt <= range.Rows.Count; rCnt++)
            {
                if ((range.Cells[rCnt + 3, 3] as Excel.Range).Value != null)
                {
                    ...
                    string[] Town = new string[rangeTown.Columns.Count - 1];
                    //In Town[], cell number 0 and 4.
                    for (cCnt = 1; cCnt <= rangeTown.Columns.Count; cCnt++)
                    {
                        int i = cCnt;
                    ...
                    string[] Township = new string[rangeTownship.Columns.Count - 1];
                    //In Township[], cell number 2 and 5.
                    for (cCnt = 1; cCnt <= rangeTownship.Columns.Count; cCnt++)
                    {
                        int i = cCnt;
